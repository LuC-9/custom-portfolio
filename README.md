...
- 🖼️ **Kinetic marquee frame** wrapping the hero portrait — two persona-aware skill / game strips run in opposite directions along the top and bottom of the image as a conveyor belt border
- 🪜 **Staircase experience section** — each career phase is a step that indents left as you go back in time, with a "Step N / total" badge and a hover-triggered SVG stick figure that jumps onto the step and points at the expanded description
- 🎯 **Compact-first cards** — the experience descriptions are hidden by default and slide open on `:hover` / `:focus-within` via the CSS `grid-template-rows: 0fr → 1fr` trick, so mouse, keyboard, and touch all reach the detail
- 🧩 **Featured bento (3×3)** — developer persona packs a 2×2 byluc.in feature card plus five 1×1 project / blog cards into a clean rectangle with no empty tiles; clicking a project opens the same rich popup the `/projects` grid uses
